...
**EDIT 2:**
There werer some confusion on whether ghost queue still keeps data. As the name suggests, the ghost queue does not keep any data, it only stores the object ID (or a hash of the object ID). A request hit on the ghost queue is a miss. 

**EDIT 3:**
There were some confusion on how to implement S3-FIFO with locks, here is a [blog post](https://blog.jasony.me/system/cache/2023/12/28/fifo).


**EDIT 4:**
Although we used two bits as frequency counter, we notice that for block I/O workloads (especially non-first layer cache), one bit is often better. 
...
<span style="font-family:Fantasy; font-variant-cap:small-caps">M</span> then uses 90% of the cache space. The ghost queue <span style="font-family:Fantasy; font-variant-cap:small-caps">G</span> stores the same number of ghost entries (no data) as <span style="font-family:Fantasy; font-variant-cap:small-caps">M</span>. Note that, a request found in the ghost queue is **not** a cache hit since the ghost queue does not contain data. Moreover, when working with VMware, we found that the Ghost queue size can be as small as 50% (instead of 90%) of the main queue size (without data). 
